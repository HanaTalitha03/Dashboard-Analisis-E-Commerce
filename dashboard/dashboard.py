...
try:
    if start_date > end_date:
        st.error("Tanggal mulai harus harus lebih besar dari tanggal akhir.")
        st.stop()

    if len(selected_state) == 0:
        st.warning("Harap pilih setidaknya satu state.")
        st.stop()
...
with col3:
    ...
    st.metric("Rata-rata Review Score", value=f"{avg_review_score:.2f}")
...
with col1:
    ...
    ax.set_title("10 State dengan Revenue Tertinggi", fontsize=15)
